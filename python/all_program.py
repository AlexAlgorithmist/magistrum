...
a = 0
c = ''
mas_reklama = ['https://ege.com', 'https://www.aviasales.ru/?etext=&params=', 'https://www.avito.ru/']
mas_otv = ['Молодец!', 'Правильно!', 'Отлично!']
mas_n_otv = ['Не получилось', 'Почти', 'Неправильно']
...
def poisk(b = '>>>'):
    ...
    if vvod == '':
        an_idiot()
        menu()
    mas_vuvod = vv.preob.poisk(vvod)
    for k in range(len(mas_vuvod)):
        print(mas_vuvod.pop())
    menu()
...
def m_test(b = '>>>'):
    ...
    if c < 1 or c > 4:
        ...
        e = str(input(f))
        if e == d[1]:
            print(vv.random.randchose(mas_otv))
        else:
            print(vv.random.randchose(mas_n_otv))
    menu()



def an_idiot():
    d = str(input('Ты идиот?\n'
                  '>>>'))
    if d == 'Да' or d == 'да':
        print('Я знал.')
    else:
